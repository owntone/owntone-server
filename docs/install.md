...
Finally, read the section [Post Installation](#post-installation) in the bottom
of this document, which describes configuration, setting up init scripts and
...
[.github/workflows/macos.yml](https://github.com/ejurgensen/forked-daapd/blob/master/.github/workflows/macos.yml)
...
Start by generating the build system by running
...
To display the configure options run
...
(See [Web Interface](webinterface.md) documentation for more informations)

Building with libwebsockets is required if you want the web interface. It will be
enabled if the library is present (with headers). Use `--without-libwebsockets`
to disable.
