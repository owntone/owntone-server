...
{
#define Q_TMPL "INSERT OR REPLACE INTO speakers (id, selected, volume, name, auth_key, password, format, offset_ms) VALUES (%" PRIi64 ", %d, %d, %Q, %Q, %Q, %d, %d);"
  char *query;

  query = sqlite3_mprintf(Q_TMPL, device->id, device->selected, device->volume, device->name, device->auth_key, device->password, device->selected_format, device->offset_ms);
...
{
#define Q_TMPL "SELECT s.selected, s.volume, s.name, s.auth_key, s.password, s.format, s.offset_ms FROM speakers s WHERE s.id = %" PRIi64 ";"
  sqlite3_stmt *stmt;
  char *query;
  int ret;
  ...
  device->password = safe_strdup((char *)sqlite3_column_text(stmt, 4));

  device->selected_format = sqlite3_column_int(stmt, 5);
  device->offset_ms = sqlite3_column_int(stmt, 6);
