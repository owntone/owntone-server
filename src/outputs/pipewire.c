...
struct pipewire_sinkctx
{
   ...
   * (either the configured pwsink_target name, or whatever
   ...
   * that once "default.audio.sink" (or pwsink_target) names a node we can
   ...
   * pwsink_target override, or whatever "default.audio.sink" metadata
   * last resolved to. Empty until known.
   */
  char                    target_name[256];

  /* Explicit "pwsink_target" config override; empty = follow the system
...
/*
 * Returns true when the pwsink volume mode is active.  Called from
 * outputs.c (outputs_device_add) to decide whether WirePlumber's own
 * persisted sink volume should take precedence over OwnTone's stored value.
 * Declared extern in outputs.c under #ifdef HAVE_PIPEWIRE.
 */
bool
pipewire_mixer_is_pwsink(void)
{
  return pipewire_mixer_mode == PIPEWIRE_MIXER_PWSINK;
}
 ...
 *     pwsink_target name, or whatever "default.audio.sink" currently names
 ...
 * using the curve configured via pwsink_volume_curve. Prefers the Route's
 ...
 * only if no pwsink_target override is configured and nothing has resolved
...
pwsink_core_reconnect(void)
{
  ...
  /* If a pwsink_target override is configured, pwsink_maybe_bind() will
...
pwsink_init(void)
{
  ...
  pwsinkctx.context = pw_context_new(pw_thread_loop_get_loop(pwsinkctx.thread_loop), NULL, 0);
  if (!pwsinkctx.context)
    {
...
pipewire_init(void)
{
  ...
  /*
   * pwsink_target: literal node.name to pin sink-volume control to,
  ...
  target = cfg_getstr(cfg_audio, "pwsink_target");
  ...
  /*
   * pwsink_volume_curve: "cubic" (default, matches wpctl/WirePlumber's own
   * perceptual scale) or "linear". Only meaningful in pwsink mode.
   */
  curve = cfg_getstr(cfg_audio, "pwsink_volume_curve");
  if (!curve || strcasecmp(curve, "cubic") == 0)
    ...
    {
      DPRINTF(E_LOG, L_LAUDIO,
        "PipeWire: unrecognized pwsink_volume_curve '%s' (expected 'cubic' or 'linear'), using 'cubic'\n",
        curve);
      pipewire_volume_curve = PIPEWIRE_CURVE_CUBIC;
    }
