...
oauth_reply_spotify(struct httpd_request *hreq)
{
  ...
  snprintf(redirect_uri, sizeof(redirect_uri), "http://owntone-server.local:%d/oauth/spotify", httpd_port);
...
oauth_reply_spotify(struct httpd_request *hreq)
{
  DPRINTF(E_LOG, L_WEB, "This version of owntone-server was built without support for Spotify\n");

  httpd_send_error(hreq->req, HTTP_NOTFOUND, "This version of owntone-server was built without support for Spotify");
