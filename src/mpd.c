 ...
 *
 * owntone-server does not support different decoders and always uses ffmpeg or libav for decoding.
...
mpd_command_playid(struct evbuffer *evbuf, int argc, char **argv, char **errmsg, struct mpd_client_ctx *ctx)
{
  ...
  if (argc > 1)
    {
      //TODO [mpd] mpd allows passing "-1" as argument and simply ignores it, owntone-server fails to convert "-1" to an unsigned int
 ...
 *
 * owntone-server only uses libav/ffmepg for decoding and does not support decoder plugins,
